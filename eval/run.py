...
def _print_summary(results: list[EvalResult]) -> None:
    ...
    click.echo("=" * 70)
    # Замер 2026-08-02: одна и та же конфигурация дала на gs008 14/15, 9/15
    # и 1/15 в трёх последовательных прогонах при побитово том же контексте.
    # temperature=0.0 стоит с самого начала — дрейфует сборка модели
    # у провайдера. Без этой строки числа отсюда читают как точные.
    click.secho(
        "ВНИМАНИЕ: одиночный прогон различает только изменения крупнее ±2 вопросов.\n"
        "Для сравнения конфигураций используйте парный замер с чередованием.\n"
        "Поиск детерминирован и мерится точно: make eval-retrieval.",
        fg="yellow",
    )
