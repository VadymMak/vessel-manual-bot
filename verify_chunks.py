...
def check_table_caption_binding(_chunks) -> list[str]:
    ...
    ТРИ ОСТАВШИХСЯ СРАБАТЫВАНИЯ НА SEBU7844-37 — НЕ ОДИН КЛАСС И НЕ «СЕТКА
    ВЫШЕ ПОДПИСИ». Тот класс двусторонняя проверка уже закрыла. Здесь три
    разные причины, и чинить их надо не здесь:

      Table 19, «Engine Operation». Предложения «Table 19 lists the ratings
        for 3500 Marine Engines…» и «Table 19 assumes the use of a fixed pitch
        propeller.» — это ПРОЗА, ссылающаяся на таблицу, но RE_TABLE_CAPTION
        (^Table\s+\d+) принимает их за подписи. Дефект классификатора:
        подписью следует считать блок, состоящий ТОЛЬКО из «Table N».

      Table 40, «Coolant Sample (Level 1) - Obtain». Сразу за подписью идёт
        «Recommended Interval Type of Coolant Level 1 Level 2» — это ШАПКА
        таблицы, но она набрана жирным в размере H2 и классифицирована как
        заголовок, из-за чего просмотр обрывается. Дефект того же рода:
        шапку таблицы нельзя отличить от подзаголовка по одному кеглю.

      Table 28, «Refill Capacities». Подпись стоит последним элементом
        группы, сетка не разобралась (rows=0), содержимого нет ни с одной
        стороны. Единственный случай из трёх, где проверка сработала
        по делу — данные таблицы действительно не собрались.

    Все три снимаются пунктом 9 очереди: если сетка будет нести
    meta["table_no"], подпись связывается с содержимым по номеру, а не
    по соседству, и ни проза, ни шапка, ни порядок элементов роли не играют.
    """
