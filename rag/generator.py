...
# ПЕРСОНА БЕЗ СЕМЕЙСТВА ДВИГАТЕЛЕЙ — ЭТО ЗАМЕР, А НЕ ВКУС.
# Прежняя строка «ассистент по судовым двигателям Caterpillar 3500B/3500C»
# была константой одного изделия при двух мануалах в базе. Парный замер
# 2026-08-02 с чередованием конфигураций внутри раунда, 20 раундов:
#     gs031  с семейством  6/20   без семейства 20/20
#            расхождений 14, ВСЕ в пользу варианта без семейства, 0 обратно
#     gs021  с семейством 17/20   без семейства 17/20   (3 против 3, шума)
# gs031 требует удержать сразу две детали процедуры («high idle» и «1 minute»),
# и лишняя рамка в шапке ему мешала. gs021 требует одну и не изменился.
# Возвращать семейство в шапку нельзя без такого же парного замера:
# блочный (все прогоны A, потом все прогоны B) здесь врёт, см. eval/retrieval.py.
